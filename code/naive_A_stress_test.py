...
def stress_test_set(i, path, name):
    ...
    for T in Ts:  # for each row of T
        ...
        for o in orbium:
            ...
            temp.to_csv("naive_nutrient_A_s"+str(int(o.dict["seed"]))+"l64L"+str(L)+"T"+str(T)+"N"+str(N)+"_stress_test.csv")
        if len(orbium) !=10:
...
#stress_test_set(iter, "naive_nutrient_A.csv", name="naive_A_")
...
def get_set(N, L, mkfile= False, parameter="seed", path="../results/naive_nutrient_A.csv"):
    """path to files, name to use for saved files"""
    df = pd.read_csv(path)
    Ts = [100, 1000, 10000]
    ...
    if mkfile:
        fgrids =["seed"]
        fgrids += grids
        with open("../results/naive_A/N"+str(N)+"L"+str(L)+"qualitative.csv", "w") as f:
...
###################### LOAD IN RESULTS ############################
def load_one(seed, path="../results/naive_A/data/", file=False):
...
def get_creature(seed, path="../results/naive_A/data/"):
    files = os.listdir(path)
    df = load_vitals(path)
    names = [n.split("_parameters.csv")[0] for n in df.files]
    df["name"] = names
...
# 451 slow sensing
# 359 weak sensing
def animate_creature(seed, s=0, standard=False, frames=500):
    orbium = get_creature(seed)
    anim = animate(orbium, orbium.enviro, seed=s, standard=standard, frames=frames)
    return anim

"""def animate_creature(seed, name=None, speedy=False, frames=700, T=None, L=None):
    orbium = get_creature(seed)
    ...
    anim = animate(orbium, orbium.enviro, frames=frames, seed=np.where(times==np.max(times))[0][0], standard=standard, name=name, **labels)
...
def load_tests(path = "../results/naive_A/data/"):
...
def load_qualitative(main_file = "../results/naive_nutrient_A.csv", path = "../results/naive_A/"):
