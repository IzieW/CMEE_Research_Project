...
#stress_test_set(iter, "depleting_nutrient_B.csv", "depleting_B_")
...
def get_set(N, L, mkfile= False, parameter="seed", path="../results/depleting_nutrient_B.csv"):
    """path to files, name to use for saved files"""
    df = pd.read_csv(path)
    Ts = [500, 1000, 10000]
    ...
    if mkfile:
        fgrids =["seed"]
        fgrids += grids
        with open("../results/depleting_B/N"+str(N)+"L"+str(L)+"qualitative.csv", "w") as f:
...
def load_tests(path = "../results/depleting_B/data/"):
...
def load_qualitative(main_file = "../results/depleting_nutrient_B.csv", path = "../results/depleting_B/"):
...
for i in range(len(means)):
    plt.errorbar([500, 1000, 10000], means.iloc[i], yerr=vars.iloc[i]/10, label=seeds[i])

plt.xlabel("Correlation Time (Tn)")
plt.ylabel("Mean survival time (timesteps)")
plt.title("Mean survival time of weak sensing creatures in environments of different correlation times")
plt.legend()
"""
