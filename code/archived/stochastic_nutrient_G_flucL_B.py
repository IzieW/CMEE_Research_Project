...
In this variation, enviro correlation length L fluctuates every g generations, randomely or stabley"""
####### IMPORTS ########
...
def get_g(file):
    """Return seed from input filename"""
    return int(re.search(r"g\d+", file).group().split("g")[1])
...
def do_render(T, N, g, path, random=False):
    Ls = [15, 20, 30]

    # Find files with T and L
    files = os.listdir(path)
    files = [i for i in files if re.search(r"parameters.csv$", i)]
    files = [i for i in files if get_N(i)==N and get_T(i)==T and get_g(i)==g and stable_or_random(i)]

    if random:
        files = [i for i in files if get_N(i)==N and get_T(i)==T and get_g(i)==g and stable_or_random(i)]
        type = "random"
    else:
        files = [i for i in files if get_N(i)==N and get_T(i)==T and get_g(i)==g and not stable_or_random(i)]
        type = "stable"
    ...
    new = []
    for L in Ls:  # recast creatures with each enviro L
        temp = deepcopy(orbium)
        for i in temp: i.enviro = StochasticEnviro(L=L, T=T)
        new += temp
    ...
    labels = {"y": [np.arange(32, (64*dim[1])+32, 64), Ls],
              ...
              "title": "N="+str(N)+", T="+str(T)+" , Enviro changing "+type+" every "+str(g)+" generations"
                       }

    render_grid(orbium, name=path+"/gifs/G_flucL_B_"+type+"_"+str(N)+"T"+str(T)+"g"+str(g)+"rendered", dim = dim, **labels)

def render_all(random):
    Ts = [500, 1000, 5000, 10000]
    Ns = [100, 1000, 10000]
    gs = [10, 100, 500]

    [do_render(T, N, g, "../results/nutrient_B/G_flucL/", random) for g in gs for N in Ns for T in Ts]
...
def update_man(creature, enviro, depletion = 0.01):
    ...
    creature.nutrition -= depletion  # minus 0.01 from nutrition for every timestep
    creature.nutrition += overlap(creature, enviro) * creature.absorption  # add nutrients from food
    creature.nutrition = min(creature.nutrition, 2)  # max they can have is 2 - gives 200 timesteps
...
def mutate_and_select(creature, enviro, k=1, moving=False, runs=10, minus_keys=1, median=False):
    ...
    def do_run(c, seed):
        ...
        c.initiate()
        return run_one(c, enviro)

    seeds = [np.random.randint(1000) for i in range(runs)]
    t_wild = np.asarray([do_run(wild_type, seeds[i]) for i in range(runs)])
    t_mutant = np.asarray([do_run(mutant, seeds[i]) for i in range(runs)])
...
def optimise(creature, obstacle, g, fluc_random= False, N=1000, k=1, seed=0, fixation_mark=600, max_time = 100, name=None, minus_key=1, median=False):
    ...
    Environment correlation length L is fluctuated every g generations"""
    ...
    while (fix < fixation_mark) and ((time.time() - start) < max_time):
        ...
        if gen % g == 0:  # Every g interval generations, update enviro L
            obstacle.i = int(gen / g)
            obstacle.update_L(random = fluc_random)
            print("L="+str(obstacle.L))
    ...
    with open(name+"_parameters.csv", "w") as f:
        ...
        csvwrite.writerow(["g", g])  # interval at which generations fluctuate
        csvwrite.writerow(["random_flux", fluc_random])  # was the fluctuation random
