...
def run_simulations(i):
    ...
    if i < 120: # Split into three and run on different population sizes
        N=100
        name = "MEDIANS_naive_B_s"+str(i)+"l64L"+str(L)+"T"+str(T)+"fixation"+str(fixation)+"N"+str(N)
        optimise(orbium, enviro, N=N, k=1, seed=i, fixation_mark=fixation, max_time = max_time, name=name, minus_key=1, median=True)  # nutrient can't be adjusted
    elif i < 240:
        N=1000
        name = "MEDIANS_naive_B_s"+str(i)+"l64L"+str(L)+"T"+str(T)+"fixation"+str(fixation)+"N"+str(N)
        optimise(orbium, enviro, N=N, k=1, seed=i, fixation_mark=fixation, max_time = max_time, name=name, minus_key=1, median=True)  # nutrient can't evolve
    else:
        N=10000
        name = "MEDIANS_naive_B_s"+str(i)+"l64L"+str(L)+"T"+str(T)+"fixation"+str(fixation)+"N"+str(N)
        optimise(orbium, enviro, N=N, k=1, seed=i, fixation_mark=fixation, max_time = max_time, name=name, minus_key=1, median=True)  # nutrient can't evolve
