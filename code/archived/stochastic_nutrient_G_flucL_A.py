...
In this variation, L fluctuates randomely or stabley every g generations"""
...
def do_render(T, N, g, path, random=False):
    ...
    render_grid(orbium, name=path+"/gifs/G_flucL_A_"+type+"_"+str(N)+"T"+str(T)+"g"+str(g)+"rendered", dim = dim, **labels)
...
def render_all(random):
    ...
    [do_render(T, N, g, "../results/nutrient_A/G_flucL/", random) for g in gs for N in Ns for T in Ts]
