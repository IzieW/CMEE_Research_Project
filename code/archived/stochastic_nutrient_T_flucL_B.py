...
In this variation, L fluctates over timesteps

fluc_random is the variable that determines if L fluctuates randomely or not"""
...
def do_render(T, path, standard=False, prep=False):
    Ns = [100, 1000, 10000]
    ...
    if prep:
        return orbiums, labels
    if standard:
        render_grid(orbiums, name=path+"/gifs/std_T_flucL_A_stable_T"+str(T)+"rendered", standard=True, dim = dim, **labels)
        render_grid(orbiumr, name=path+"/gifs/std_T_flucL_A_random_T"+str(T)+"rendered", standard=True, dim = dim, **labelr)
    else:
        render_grid(orbiums, name=path+"/gifs/T_flucL_A_stable_T"+str(T)+"rendered", dim = dim, **labels)
        render_grid(orbiumr, name=path+"/gifs/T_flucL_A_random_T"+str(T)+"rendered", dim = dim, **labelr)


def update_reorg(i):
    global img
    # update board, and creatures
    i += ((creatures[0].enviro.T/2)-100) # bring right to moment of reorder
...
def reorg(T, name=None, standard=False):
    """Render the reorganisation"""
    temp = do_render(T, "../results/nutrient_B/T_flucL/", prep=True)
    labels = temp[1]
    orbium = temp[0]

    # Update until 100 timesteps right before change of direction
    for t in range(int((T/2)-100)):
        for c in orbium:
            if t % c.enviro.dt == 0:
                c.enviro.update()
            update_man(c, c.enviro)
            
    globals().update({"creatures":orbium,
    "dim": [10,3],
    "fluc_random": True})
    ...
    if standard:  # Standardise to show 10,000 timesteps (jumping)
        ...
        anim = animation.FuncAnimation(fig, update_grid, frames=400, interval=20)
    if name:
...
def render_all():
    ...
    [do_render(T, "../results/nutrient_B/T_flucL/") for T in Ts]
...
def update_grid_std(i):
    global img
    i *=25
    for t in range(i, i+25):
    # update board, and creatures
        for n in range(len(creatures)):
            if t % (creatures[n].enviro.T/2) == 0:
                creatures[n].enviro.update_L(random = fluc_random)
            if t % creatures[n].enviro.dt == 0:
                creatures[n].enviro.update()
            update_man(creatures[n], creatures[n].enviro)
