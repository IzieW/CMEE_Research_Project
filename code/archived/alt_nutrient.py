...
"""Evolve orbium in new nutrient environment.

Orbium have nutrition value which is in constant decline. Absorption of
nutrients can restore it back to one, where it needs to be for survival.

NOVEL about this nutrient- no growth mean involved and the creature can eat as much
as it wants to. Just like bacteria. """
...
def get_size(creature):
    return np.sum(creature.A > 0)  # get sizes that are occupied
...
def prob_fixation(wild_time, mutant_time, N, k):
...
def update_man(creature, food, depletion = 0.01):
    """Update creature by 1/T according to values in creature channel and environment.
    Update nutrition levels by constant decay + increase by food"""
    creature.nutrition -= depletion  # minus 0.01 from nutrition for every timestep
    creature.nutrition += overlap(creature, food) * creature.absorption  # add nutrients from food
...
def run_one(creature, enviro, moving=False, verbose=False, give_sums=False, return_means=False):
