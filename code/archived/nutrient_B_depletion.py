...
In this variation, orbium are depleted proportional to bodymass"""
####### IMPORTS ########
...
def update_man(creature, enviro):
    ...
    creature.nutrition -= 0.0002*(np.sum(creature.A>0)**.75)  # depleted proportional to bodymass
    creature.nutrition += overlap(creature, enviro) * creature.absorption  # add nutrients from food
    creature.nutrition = min(creature.nutrition, 1)  # max they can have is 2 - gives 200 timesteps
...
def optimise(creature, obstacle, N=1000, k=1, seed=0, fixation_mark=600, max_time = 100, name=None, minus_key=1, median=False):
    ...
    start = time.time()
    dfit = 10 # set dfit
    while (dfit > 1/N) and ((time.time() - start) < max_time):
        ...
        if gen >= 3:
            if (time_log.wild_mean.iloc[-3:].mean()== 10000) and not time_log.wild_mean.iloc[-3:].var():
                dfit = 0 # terminate when reach max out of 10000
...
def test_nut(orbium, absorption=0.00003, means=False):
    ...
    for i in range(10):
        orbium.initiate()
        m[i] = run_one(orbium, enviro, return_means=means)
    return (m.mean(), m.var())
