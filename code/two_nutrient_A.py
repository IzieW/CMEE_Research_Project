...
def sort(files, Ns, Ls):
    ...
    final = []
    for N in Ns:
        f = [i for i in files if get_N(i)==N]
        for L in Ls:
...
def plot_s(creatures, dat="selection"):
    ...
    plt.ylabel(dat)
    plt.title("Multiple A"+dat+"over time")
...
def selection(c, t_wild, t_mutant, k):
    ...
    if pfix >= np.random.uniform(0, 1):
        # ACCEPT MUTATION
        c.s_winning.append(c.selection_log[-1]) # record winning s
        return True
    else:
        # REJECT MUTATION
        return False
...
def metabolism(creature, enviro):
    ...
    creature.running_mean = creature.running_mean + (
                overlap(creature, enviro, sums=True) * creature.absorption)
...
def update_man(c1, c2, enviro):
    ...
    # If creatures touch, cells die immediately
    temp = deepcopy(c1)
    c1.A -= overlap(c1, c2, sums=False, two_creatures=True)
    c2.A -= overlap(c2, temp, sums=False, two_creatures=True)

    # Update nutrients and board
    for c in [c1, c2]:
        metabolism(c, enviro)  # update nutrient values
        U = np.real(np.fft.ifft2(c.K * np.fft.fft2(c.A)))
        c.A = np.clip(c.A + 1 / c.T * c.growth(U), 0, 1)
