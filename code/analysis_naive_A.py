...
markers = ["o", "v", "^", "s", "p", "P", "*",
           "X", "x", "1", "D", "d"]
name = "../results/naive_A_solution_count.csv"
df = pd.read_csv("../results/naive_nutrient_A.csv")
times = pd.read_csv("../results/naive_A_means.csv")
counts = pd.read_csv("../results/naive_A_counts_by_solution.csv")

tukey = pd.read_csv("../results/naive_A_tukey.csv")
...
qual_labels = ["rings", "perfect swimmers", "abnormal swimmers", "weak sensing", "sensing rings", "pulsing", "fluid cells", "radial", "slow sensing", "slow dissolving", "projection", "ghost"]
...
def plot_survival_by_N(df=df, log=False):
    d = df
    d = d.groupby("N").describe().survival_mean
    if log:
        plt.errorbar(np.log10(d.index), d["mean"], yerr=d["std"]/np.sqrt(d["count"]), capsize=4)
    else:
        plt.errorbar(d.index, d["mean"], yerr=d["std"]/np.sqrt(d["count"]), capsize=4)
    plt.ylabel("Mean survival time")
    plt.xlabel("Log10 Population size (N)")
...
def plot_all_parameter_space2(df=df, orbium=False, R=False):
    """3D plot of L~T for a given variable z. If means is false, will plot for each population size N"""
    d = df[df.qualitative != "NA"]
    cmap = matplotlib.cm.get_cmap('hsv')
    forms = d.qualitative.unique()
    ...
    if R:
        ...
        ax.set_zlabel("log T")
    ax.set_title("Average parameter space across all solutions")
...
def plot_all_forms_by_enviro(enviro, df=df):
    df = df[df.qualitative != "NA"]
    for form in df.qualitative.unique():
        d = df[df.qualitative==form]
        d = d.groupby(enviro).describe().R["count"]
        plt.plot(d.index, d.values, label = form)
    plt.ylabel("Count")
    plt.xlabel(enviro)
    plt.legend(bbox_to_anchor=(1.05, 1))
...
def scatter_log_parameter_space(df=df, orbium=False, R=False):
    ...
    for form in forms:
        ...
        i+=1
    if orbium:
        ax.scatter3D(np.log(0.15), np.log(0.0015), np.log(13), label="orbium", c="r")
    #ax.contourf(np.log(Ls),  np.log10(Ts), z, levels=500, alpha = 0.5)
    #ax.contourf(np.log(Ls),  np.log10(Ts), get_table(N=100,d=d), levels=500, alpha=0.5)
    ax.set_xlabel("$log_{e}$ Growth mean (m)")
    ax.set_ylabel("$log_{e}$ Growth std (s)")
    ax.set_zlabel("$log_{e}$ R")
    ...
    ax.set_xlabel("$log_{e}$ Growth Mean (m)")
    ax.set_ylabel("$log_{e}$ Growth Std (s)")
    ax.set_zlabel("$log_{e}$ T")
    plt.suptitle("Nutrient A: Qualitative behaviours in $log_{e}$ parameter space")
    plt.legend(bbox_to_anchor=(1.45, .90), title="Qualitative behaviour", labels=qual_labels)


def percentages(df=df):
    df = df[df.qualitative != "NA"]
    for form in df.qualitative.unique():
        print(form, len(df[df.qualitative==form])/len(df)*100)


def percentages_by_enviro(df=df):
    df = df[df.qualitative != "NA"]
    for form in df.qualitative.unique():
        print(form, len(df[df.qualitative==form])/len(df)*100)
...
def test_groups(df=df):
    ...
    Ps = []
    for parameter in []:
        for forms in df.qualitative.unique():
            d1 = df[df.qualitative == form]
            d2 = df[df.qualitative != form]
            forms.append(form)
            temp = f_test(d1, d2)


def plot_mass_forms(df=df):
    df = df[df.qualitative != "NA"]
    for form in df.qualitative.unique():
        d = df[df.qualitative==form]
        plt.scatter(np.log(d.mean_volume), np.log(d.mean_mass), label=form)
        plt.legend()
        plt.ylabel("log mean mass")
        plt.xlabel("log mean volume")

def plot_var_centroid_forms(df):
    df = df[df.qualitative != "NA"]
    for form in df.qualitative.unique():
        d = df[df.qualitative==form]
        plt.scatter(np.log(d.mean_volume), np.log(d.mean_mass), label=form)
