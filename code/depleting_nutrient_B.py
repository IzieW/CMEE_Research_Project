...
def animate(creature, enviro, name=None, seed=0, frames=400, interval=20, standard=False, **labels):
    ...
    fig = figure_world(np.dstack([np.zeros([64,64]), creature.A, enviro.grid]), **labels)
    print("rendering animation...")
    if standard:
...
def update_man(creature, enviro):
    ...
    creature.nutrition = min(creature.nutrition, 1)  # if not getting more than 0.1, they are starving
